...
<body class="bg-gray-50">
    <div class="min-h-screen">
        <header class="bg-blue-600 text-white shadow-lg">
            <div class="max-w-7xl mx-auto px-4 py-6">
                ...
                <p class="text-blue-200 mt-1">
                    <?php echo \App\Models\Address::count(); ?> 条数据 ·
                    <?php echo \App\Models\Address::where('level', 'province')->count(); ?> 个省级行政区
                </p>
            </div>
        </header>
        ...
        const provinceData = <?php
             = \App\Models\Address::with(['children.children' => function(){
                ->orderBy('sort');
        ...
        function renderTree(nodes, depth) {
            ...
            for (var i = 0; i < nodes.length; i++) {
                ...
                html += '<span style="padding-left:' + padding + 'px" class="flex-1 font-medium ' + (depth === 0 ? 'text-lg' : 'text-base') + '">';
        ...
        function resetSearch() {
            ...
            var header = document.querySelector('main > h2');
            if (header) header.remove();
        }

        function getAllData(nodes, depth) {
            var results = [];
            for (var i = 0; i < nodes.length; i++) {
                var node = nodes[i];
                results.push({name: node.name, code: node.code, depth: depth});
                if (node.children && node.children.length > 0) {
                    var children = getAllData(node.children, depth + 1);
        ...
        function buildResultChain(chain) {
            ...
            if (chain.length > 1) {
                var childResult = buildResultChain(chain.slice(1));
        ...
        function doSearch() {
            ...
            var allFlat = getAllData(provinceData, 0);
            var matchedCodes = [];
            ...
            for (var j = 0; j < matchedCodes.length; j++) {
                ...
                if (chain) {
                    var result = buildResultChain(chain);
                    resultMap[chain[0].code] = result;
                }
            }
            for (var k = 0; k < matchedCodes.length; k++) {
                var chain = findParentChain(provinceData, matchedCodes[k], null);
                if (chain && chain[0].children === undefined) {
                    var rootResult = buildResultChain(chain);
                    if (rootResult && rootResult.children.length === 0 && !resultTree.some(function(x){return x.code === rootResult.code;})) {
                        resultTree.push(rootResult);
                    }
                }
            }
            if (resultTree.length === 0) {
                for (var m = 0; m < matchedCodes.length; m++) {
                    var chain2 = findParentChain(provinceData, matchedCodes[m], null);
                    if (chain2) {
                        var r2 = buildResultChain(chain2);
                        var code = r2.code;
                        if (!resultTree.some(function(x){return x.code === code;})) {
                            resultTree.push(r2);
                        }
                    }
                }
            }
            renderSearchResults(resultTree);
            document.getElementById('searchHint').textContent = '搜索结果（共 ' + matchedCodes.length + ' 条命中，含所有上级地址）';
        }

        function renderSearchResults(nodes) {
            if (nodes.length === 0) {
                return '<div class="bg-white rounded-lg shadow p-8 text-center text-gray-500"><p class="text-lg">未找到匹配的结果</p></div>';
            }
            var html = '<h2 class="text-lg font-bold mb-4">搜索结果</h2>';
        ...
        function renderSearchNode(node, depth) {
            ...
            html += '<span class="text-xs mr-2 opacity-50">[' + levelLabel + ']</span>';
            html += '<span style="padding-left:' + padding + 'px" class="flex-1 font-medium text-base">';
            html += '<span class="mr-2">' + icon + '</span>' + node.name;
            html += '</span>';
